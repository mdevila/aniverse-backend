...
async function getConversionStatus(videoId) {
  ...
  // Try ytdl-core first (fast path — ~2-3 seconds)
  const state = progressState.get(videoId);
  if (!state) {
    progressState.set(videoId, { progress: 0.1, downloadUrl: null, method: 'ytdl' });

    try {
      const audio = await extractAudioUrl(videoId);
      if (audio?.url) {
        cache.set(videoId, { url: audio.url, ts: Date.now() });
        progressState.set(videoId, { progress: 1, downloadUrl: audio.url, method: 'ytdl' });
        return { status: 'done', progress: 1, downloadUrl: audio.url };
      }
    } catch (e) {
      console.warn('ytdl-core failed, falling back to loader.to:', e.message);
    }

    // Fallback: start loader.to conversion
    try {
...
async function pollInBackground(videoId) {
  const state = progressState.get(videoId);
  if (!state || !state.progressUrls) return;
