...
async function getConversionStatus(videoId) {
  ...
  // Fallback: start loader.to jobs, return progressUrls for frontend to poll directly
  try {
    // Start 3 parallel jobs for maximum speed
    ...
    const unique = [...new Map(valid.map(j => [j.id, j])).values()];

    return {
      status: 'converting',
      progress: 0,
      downloadUrl: null,
      progressUrls: unique.map(j => j.progressUrl),
    };
  } catch {}
