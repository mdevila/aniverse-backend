...
// Try Puppeteer if available, otherwise use HTTP-based extraction
let puppeteer = null;
...
// Full flow: try HTTP first (fast), then Puppeteer (reliable)
async function extractVideoStream(animeSession, episodeSession) {
  ...
  // Try each source — HTTP method first (fast, no browser)
  for (const source of sources) {
    try {
      const videoUrl = await extractVideoHttp(source.url);
      if (videoUrl) {
  ...
  // Fallback: Puppeteer for first source
  for (const source of sources) {
    try {
